...
> 状态:**DONE**(2026-06-13 落地;立项 2026-06-12)。用户「把 F305 处理了吧」。
> 类型:架构决策记录 + 已执行工单 · 优先级 P2
>
> **落地摘要(2026-06-13)**:① 新增 validator `R-LAYOUT-DEPRECATED`(warn_soft ·
> advisory · 真源=`window.__DECK_JSON__` 的 authored layout,非渲染后 data-layout ·
> imported/无 deck.json 豁免 · 仅 scope 内页报);② 退役反向规则 `R-RAW-LOOKS-SCHEMA`
> (与 raw-first 立场冲突——用户三选一拍板「退役」);③ 文档单源收窄(design-first 回退表
> /policy/SKILL Hard Gate #3/schema 描述);④ 模板冻结面包屑 `templates/DEPRECATED-F305.md`
> (不参与渲染,零客户泄漏)。全程零回归:覆盖闸 84/84/84、rule-contract/check-only-gate
> (F-03)/golden-gate/examples 视觉基线/atomic-render 全过;新增 `test_layout_deprecated.py`
> 10 测(8 引擎行为 + 2 静态契约)全过;真管线渲 phase-1b-demo 实测 8 冻结页报 advisory、
> 5 仪式页零报、render PASS 不阻塞。**deck.json 保留裁决**(见 §0.1)无代码改动,记录在案。
> 顺带补全了今天另一 session 半成品提交 `a06f171` 漏登记的 `R-VIS-SVG-TEXT-FLOOR`
> (FAMILIES/yaml/references/视觉基线四处,本是 main 上既存红闸)。
...
- [x] 冻结版式照常渲染零回归(phase-1b-demo 实测 render PASS + golden-gate/examples
      视觉基线/atomic-render 全过)。
- [x] 新页用冻结版式 → `R-LAYOUT-DEPRECATED` 出现(phase-1b-demo 8 冻结页实测命中);
      仪式页(cover/agenda/section/quote/end)→ 不报(实测 5 仪式页零报);imported /
      无 deck.json → 整体豁免(test_layout_deprecated 实测)。**永不阻塞**(warn_soft)。
- [x] 三处文档口径一致且互引:deck-generation-policy.md / design-first.md /
      SKILL.md Hard Gate #3(+ schema 描述 + templates/DEPRECATED-F305.md 面包屑)。
- [x] 退役 `R-RAW-LOOKS-SCHEMA` 全牵连面清理(规则体/helper/RULE_META/FAMILIES/yaml/
      references/注入注释),覆盖闸 84/84/84 对齐,无死码。

> 注:原 §3.2「模板文件头部加注释」改为 `templates/DEPRECATED-F305.md` 单文件面包屑——
> 模板引擎是自定义 regex 渲染器(无注释机制),`{# #}`/`{{! }}` 会泄漏、HTML 注释会渲进
> 客户可见输出,故不在 18 个 fragment 里逐个塞。冻结清单与铁律集中在该面包屑文件。
