...
class SelfContainedAuditTest(unittest.TestCase):
    def test_head_style_referencing_slide_key_fires(self):
        html = HEAD.format(
            style='<style id="fs-deck-page-anim">.slide[data-slide-key="hero"] .x{opacity:1}</style>',
            body=FRAME.format(k="hero", inner="<h1>hi</h1>"))
        soft, hard = _run(html)
        self.assertIn("R-SELF-CONTAINED", soft)
        self.assertEqual(hard, [])   # advisory only — never an error/warning

    def test_head_style_referencing_data_page_fires(self):
        html = HEAD.format(
    ...
    def test_co_located_custom_css_inside_slide_does_not_fire(self):
        ...
        soft, _ = _run(html)
        self.assertNotIn("R-SELF-CONTAINED", soft)

    def test_framework_inlined_block_does_not_fire(self):
    ...
    def test_head_style_without_perslide_selector_does_not_fire(self):
