...
| **L6** | tree-shaker 通用化(`lift-slides.py --shake`):`[data-layout=X]` 抽取从硬编码 5-heavy 扩到**该 slide 真实 layout(任意 ~15,含 extra-layouts/patterns 三表)** + **source-head per-slide 规则恢复**(把源 head `<style>` 里 `[data-slide-key]`/`[data-page]` 命中本页的规则搬进来,`[data-page=N]`→`[data-slide-key]` 改写)+ **`@keyframes` 闭包**。**效果:任何老 deck 直接 `--shake` 切干净,无需先跑 codemod**(向后兼容)。**故意不做全局 class 扫描**(实测 `.ns-card` 等是全局 `.slide .foo` 规则,任何 link feishu-deck.css 的目标 deck 都在,避免特异性/retheme 风险)。无 `--shake` 时对非 heavy layout 打印 hint;legacy 行为不变 | M | ✅ **本次完成** |
