...
- **Step 4 · 闸门 + 落盘**:beyond-default 页→等确认;全 schema→宣告即走。确认后:PREFLIGHT → new-run → **把锁定方案写 `runs/<ts>/output/DESIGN-PLAN.md`**(与 PROMPTS/texts.md 同级),生成严格照它走;偏离先回来改 plan,不静默漂移。
