...
| lifted 污染 CSS/脚本 | lifted raw `data.html` 只放结构内容;作者 CSS 进 `custom_css`,脚本不进 deck。 | `lift-slides.py` / `deck-cli paste` hygiene + 人工复核 | 「源页就是这么渲染的,原样搬最忠实」→ `.slide/.card/h1/.stage` 这类宽选择器进 `data.html <style>` 会污染整 deck;工具会自动收敛到 `custom_css` 并剥 `<script>`/`on*=`,坏了重建页面,别把污染源补回去。 | `subskills/editor/SKILL.md` → Lift hygiene;`assets/lift-slides.py` |
