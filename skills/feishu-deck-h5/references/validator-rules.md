...
| White-text | R-WHITE-TEXT | semantic body text on dark slides is `#fff` not low-opacity gray (which vanishes on projector); chrome opt-out via `/* allow:white-opacity */` (author `<style>` rules) or `data-allow-white-opacity` (DOM attr). Scans BOTH author `<style>` CSS rules AND inline `style="color:rgba(255,255,255,<1)"` attributes (the inline pass honors the `font-size<=14` chrome floor and `data-allow-white-opacity`; emits an `<inline>` finding) — parity with the old `audit_white_text` which also scanned inline style |
