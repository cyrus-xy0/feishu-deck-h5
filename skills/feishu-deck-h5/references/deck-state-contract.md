...
| `deck-json/lift-swap.py` | the complete destination deck directory bundle | **Plan-first two-endpoint guard.** Default and legacy positional invocations are read-only. A write requires named `--source` / `--target`, `--apply`, and the current plan token. It fingerprints source control files, stages the destination, preserves target key/order/count, requires lifted provenance plus a successful scoped render and non-empty screenshot, then atomically swaps the destination only if its original tree fingerprint is unchanged. Missing/stale tokens, ambiguous/same-deck direction, source mutation, render failure, or concurrent target mutation leave the official destination unchanged. |
...
`lift-swap.py` without `--apply` (default plan).
