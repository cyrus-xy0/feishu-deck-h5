...
- 用户明确要把 PPT/PPTX 拆成可复用素材或迁移页面时,仍先触发 parser。
  `.pptx` 页面/内容导入走 build_pptx → 结构化 `canvas` deck.json(代码重建、无截图);
  `.key` 只登记来源并明确提示原生转换已退役,要求用户提供 `.pptx` 或 `.pdf`。
...
`pptx-to-deck`**
里的 `build_pptx.py`。Parser 会探测 `FS_DECK_PPTX_PYTHON`、skill 自有 venv 和
系统 Python,选择能导入 `python-pptx` + `lxml` 的解释器,把每页 OOXML
逐元素**代码重建**成一个
...
- 调用:`parse.py` 自动解析解释器后执行 `<python> <skills>/pptx-to-deck/assets/build_pptx.py
...
- **图片/双背景路线已整条退役**(用户决定:完全不要图)。不要再用
  ...
  再走已退役的 Keynote 抽图链路——`.pptx` 页面导入一律走 build_pptx 结构化重建;
...
原生 `.key` 转换没有可执行后端。Parser 只保留原文件、哈希和 provenance,
写入 `processing_status: unsupported`,然后要求用户提供:

- `.pptx`:需要结构化、可编辑页面时;
- `.pdf`:接受逐页 replica 时。

不得生成假的成功 dossier,不得引用或尝试调用已删除的 Keynote 后端。
...
- PPT:登记为需转换来源,建议转 PPTX / PDF 后继续解析。
- Keynote `.key`:仅登记来源并返回 `unsupported`;要求用户补 `.pptx` 或 `.pdf`。
- PDF:统计页数,保留来源和页序。
...
1. **分类上传物**
   ...
   - Keynote `.key`:原生转换已退役;只登记 provenance,要求补 `.pptx` 或 `.pdf`,不尝试调用旧后端。
