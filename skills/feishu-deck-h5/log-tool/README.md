...
runs/<deck>/
  ...
  log/                       ← deck-log 的地盘
    ...
    versions/vNNN/snapshot.html   每版冻结副本(注入 <base> 可独立打开)
...
| `version` | `snapshot` | `v`,`label`,`slides[]`,`snapshot` | 一个版本:整套截图 + 冻结副本 |
...
$DL snapshot <deck-dir> --label "首版骨架"      # 每出一版:冻结 + 截全套图 + 跑校验
