...
$DL on | off | status                            # 全局自动 making-of 开关(默认关;on 写 ~/.claude/deck-log.on → render 自动 snapshot+捞回复)
```
...
- **默认关(2026-06-21)**:`render-deck.py` 不再自动拍 making-of(省 render 时间)。
  `deck-log on`(写 `~/.claude/deck-log.on`)开启后,每次 render 自动给有 `log/` 的 deck
  snapshot 并捞 transcript;手动 `deck-log snapshot` 任何时候都能用,不受此开关约束。
- `deck-log off`(写 `~/.claude/deck-log.off`,凌驾一切)→ 硬关,连手动 snapshot 也不捞
  transcript。`~/.claude/deck-log.active` 仅供 `status` 查看当前活跃 deck。
