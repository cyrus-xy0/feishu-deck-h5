...
bash assets/run.sh <in.pptx> <out-dir> [选项]
...
运行 `bash assets/bootstrap.sh` 会在 `./.venv` 安装 `python-pptx`、`lxml`
（已不需要 PyMuPDF / LibreOffice）。也可设置 `FS_DECK_PPTX_PYTHON` 指向已有依赖的解释器。
